...
fun main(){
    ...
    /**/
    // null이 가능한 문자열(nullable string)
    // 형식 : 타입명뒤에 ? -> ?표시가 비어있는 값을 넣을 수 있게 해준다.
    ...
    // !!.
    // 역할 : nullable 타입 -> non-null 타입 변경. nullable 타입이 null 값 -> nullpointerexception 발생
...
// Kotlin에서는 <변수가 null을 값으로 가질 수 있는지 아닌지> 선언할 능력을 타입 시스템으로 지원
// -> 컴파일러가 컴파일리에서 발생한 Null 포인터 제외 오류 찾아내고 실행될 확률 낮춰